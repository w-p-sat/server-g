...
// --- Нова функція: центро-орієнтований рух ---
function meanRevert(current, target, volatility = 3) {
    const pull = (target - current) * 0.1; // сила тяжіння до центру
    return current + pull + getRandom(-volatility, volatility);
}
...
async function updateData(gameId) {
    ...
    // Логіка переходу між фазами + нова поведінка
    switch (phase) {
        case PHASES.HIGH_STABILITY:
            if (elapsedMinutes > phaseDurationMinutes) {
                state.phase = PHASES.TRANSITION_DOWN;
            ...
            newPrice = meanRevert(currentPrice, 75, 3);
            break;
        ...
        case PHASES.TRANSITION_DOWN:
            ...
            newPrice = meanRevert(currentPrice, 25, 4); // рух вниз
            break;
        ...
        case PHASES.LOW_STABILITY:
            ...
            newPrice = meanRevert(currentPrice, 25, 3);
            break;
        ...
        case PHASES.TRANSITION_UP:
            ...
            newPrice = meanRevert(currentPrice, 75, 4); // рух вгору
            break;
    }

    // Обмежуємо у безпечному діапазоні, але без "залипання"
    if (newPrice > 95) newPrice = 94 - getRandom(0, 2);
    if (newPrice < 10) newPrice = 11 + getRandom(0, 2);
